...
async fn test_filter_single_day_monday(pool: PgPool) {
    ...
    assert!(crns.contains(&"10007".to_owned()), "PHYS with MWF meeting should match Monday");
    assert!(!crns.contains(&"10002".to_owned()), "TTh course should not match Monday");
    assert!(!crns.contains(&"10005".to_owned()), "Saturday course should not match Monday");
    assert!(!crns.contains(&"10006".to_owned()), "TBA course should not match Monday");
}
...
async fn test_filter_multi_day_and_semantics(pool: PgPool) {
    ...
    assert!(crns.contains(&"10007".to_owned()), "PHYS with MWF meeting should match MWF filter");
    assert!(
...
async fn test_filter_tuesday_thursday(pool: PgPool) {
    ...
    assert!(crns.contains(&"10007".to_owned()), "PHYS with TTh meeting should match TTh filter");
}
